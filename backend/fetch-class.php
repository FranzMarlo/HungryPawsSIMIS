...
class fetchClass extends db_connect
{
    ...
    public function getStaffProducts($branch_id)
    {
        $query = $this->conn->prepare(
            "SELECT 
                    p.product_id,
    ...
    public function getLowStockItems($branch_id)
    {
        $query = $this->conn->prepare("
            SELECT 
                p.product_name,
    ...
    public function getStockRequestAlert($branch_id)
    {
        $query = $this->conn->prepare("
            SELECT 
                p.product_name,
            ...
            FROM inventory i
            JOIN product p ON i.product_id = p.product_id
            JOIN stock_transfer s ON p.product_id = s.product_id
            JOIN branch b ON s.receiving_branch_id = b.branch_id
            WHERE s.sending_branch_id = ? 
    ...
    public function getInventoryProducts($branch_id)
    {
        $query = $this->conn->prepare(
            "SELECT
                    i.inventory_id,
    ...
    public function getStockRequests($branch_id)
    {
        $query = $this->conn->prepare(
            "SELECT
                    t.transfer_id, 
                    ...
                    p.product_name,
                    t.quantity,
                    b.branch_name as sending_branch,
                    t.transfer_date,
                    t.status
    ...
    public function getGroomingList($branch_id)
    {
        $query = $this->conn->prepare(
            "SELECT
                    g.service_id, 
    ...
    public function getBookingList($branch_id)
    {
        $query = $this->conn->prepare(
            "SELECT
                    h.booking_id, 
    ...
    public function getManagerStockRequests($branch_id)
    {
        $query = $this->conn->prepare(
            "SELECT
                    t.transfer_id, 
                ...
                INNER JOIN product AS p 
                    ON t.product_id = p.product_id
                INNER JOIN branch AS b
                    ON t.receiving_branch_id = b.branch_id
                WHERE t.sending_branch_id = ?
                ORDER BY t.transfer_date DESC;
                "
        );
        $query->bind_param('s', $branch_id);
