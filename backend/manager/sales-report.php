...
if (empty($branchId)) {
    ...
    $salesPerformance = $fetch->getGlobalSalesPerformanceReport($startDate, $endDate);
    $topProducts = $fetch->getGlobalTopProducts($startDate, $endDate);
    $paymentMethods = $fetch->getGlobalPaymentMethodBreakdown($startDate, $endDate);
    $categories = $fetch->getGlobalCategoryBreakdown($startDate, $endDate);
